...
export default function Breakfast() {
  ...
  useEffect(() => {
    fetch("https://y-one-rose.vercel.app/breakmeals")
      .then(res => res.json())
 ...
 const handleSort = (e) => {
  ...
  if (text === "low") {
    fetch("https://y-one-rose.vercel.app/breakmeals/sort/low-high")
      .then(res => res.json())
      .then(data => setMeals(data))
  ...
  if (text === "high") {
    fetch("https://y-one-rose.vercel.app/breakmeals/sort/high-low")
      .then(res => res.json())
      .then(data => setMeals(data))
      .catch(err => console.error(err));
  }
  ...
  return (
    <div>
      <h2 className="text-3xl sm:text-5xl font-bold fontext mt-20 mb-8 text-center">Breakfast Meals</h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3  gap-10">
        {meals.map((meal) => (
          <div key={meal._id} className="relative mx-auto w-full max-w-md bg-green-900 rounded-2xl sm:rounded-t-full shadow-xl hover:bg-green-950 cursor-pointer hover:shadow-2xl transition-all duration-300 border border-gray-200 overflow-hidden ">
